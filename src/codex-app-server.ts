...
- Escreva apenas os dados em edit/remotion/public/: edit-data.json (a edicao inteira), captions.json (palavras da transcricao), segments.json (cortes), caption-cues.json (so para a legenda empilhada) e track.json. O scaffold ja deixa versoes neutras de captions.json, segments.json e track.json, entao o projeto sempre compila; substitua as que a edicao precisar.
- Os arquivos de legenda tem geradores oficiais em EDVID_HELPERS, e voce deve usa-los em vez de montar o JSON na mao: python3 "$EDVID_HELPERS/captions_for_remotion.py" --transcript <transcricao.json> -o public/captions.json, e para a legenda empilhada tambem python3 "$EDVID_HELPERS/caption_style.py" --transcript <transcricao.json> -o public/caption-cues.json --lang pt. Eles aceitam direto o JSON do WhisperX. Sem o caption-cues.json a legenda empilhada nao aparece.
- Quando o briefing pedir tracking de rosto, gere o track.json com python3 "$EDVID_HELPERS/face_track.py" <cut.mp4> -o public/track.json. Com o tracking desligado, deixe o track.json neutro que ja veio no scaffold.
- O segments.json deve vir dos limites reais dos cortes (ffprobe -count_frames nos clipes), nunca da soma dos segundos do EDL. Nunca edite src/Main.tsx.
